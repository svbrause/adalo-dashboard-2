/**
 * Demo patient for Gravitas Medspa (`Gravitas272`) when the dashboard has few
 * or no live Airtable rows yet.
 * Tanya Tan mirrors the Admin / Slim Studio showcase (Aura turntable, severity
 * scores, completed Gravitas skincare quiz with Clear Skin Guide routines).
 *
 * Merged for Gravitas logins (dev and production); disable with
 * VITE_GRAVITAS_SAMPLE_CLIENTS=false.
 * Treatment plan edits persist in sessionStorage (ids `gravitas-demo-*`).
 */
...
const TANYA_TAN_SEVERITY: AnalysisSeverityScoresData = {
  ...
  submission_id: "gravitas-demo-tanya",
};
...
function baseClient(overrides: Partial<Client> & Pick<Client, "id" | "name">): Client {
  ...
  return {
    email: `${overrides.id}@demo.gravitas.local`,
    phone: "+1 310 555 2293",
    zipCode: "91316",
    age: 38,
    ageRange: "30-39",
    dateOfBirth: null,
    ...
    locationName: "Gravitas Medspa — Encino (demo)",
...
export function filterOutGravitasSamplesDuplicatedByName(
  liveClients: Client[],
  samples: Client[],
): Client[] {
...
/** True when Gravitas demo patients should be injected (default-on for that provider). */
export function isGravitasSampleClientInjectionEnabled(
  provider?: { code?: string | null; id?: string | null; name?: string | null } | null,
): boolean {
  const v = import.meta.env.VITE_GRAVITAS_SAMPLE_CLIENTS as string | undefined;
  if (v === "false" || v === "0") return false;
  if (v === "true" || v === "1") return true;
  if (isGravitasProvider(provider ?? null)) return true;
  return Boolean(import.meta.env.DEV);
}

export function getGravitasSampleClients(): Client[] {
  return [
    baseClient({
      id: "gravitas-demo-tanya",
      name: "Tanya Tan",
      age: 38,
      ageRange: "30-39",
      phone: "+1 310 555 8803",
      ...
      aestheticGoals:
        "Brighten uneven tone with Clear Skin Guide home care and in-office brightening",
      facialAnalysisStatus: "complete",
      severityScoresFromAnalyses: TANYA_TAN_SEVERITY,
      demoFacialAnalysisAi: TANYA_TAN_ANALYSIS_AI,
      skincareQuiz: GRAVITAS_SKINCARE_QUIZ,
      discussedItems: items(
        {
          id: "gravitas-tanya-d1",
          treatment: "Medical Skin Services",
        ...
        {
          id: "gravitas-tanya-d2",
          treatment: "Medical Skin Services",
          product: "Skinpen Microneedling",
        ...
        {
          id: "gravitas-tanya-d3",
          treatment: "Facials",
          product: "Skin Vitalizing Experience",
          interest: "Dark Spots",
        ...
        {
          id: "gravitas-tanya-d4",
          treatment: "Skincare",
          product: "Powerhouse Vit. C Plus",
          interest: "Dark Spots",
        ...
        {
          id: "gravitas-tanya-d5",
          treatment: "Skincare",
          product: "Biome Balance",
          interest: "Uneven skin texture",
          findings: ["Fine Lines"],
          region: "Full face",
          timeline: "Now",
        ...
        {
          id: "gravitas-tanya-d6",
          treatment: "IV Therapy",
          product: "Ultra C Wellness Drip",
          interest: "Dark Spots",
...
export function getGravitasSampleClientsIfEnabled(
  provider: { code?: string | null; id?: string | null; name?: string | null } | undefined,
): Client[] {
  if (!isGravitasProvider(provider ?? null)) return [];
  if (!isGravitasSampleClientInjectionEnabled(provider)) return [];
  return getGravitasSampleClients().filter((c) => c.tableSource === "Patients");
}

export { GRAVITAS_PROVIDER_CODE };
