...
# Turntable can be rendered above MV / GSLRM resolution (3DGS is continuous in space).
# 150 square frames at 1024² — ~2 GB peak GPU during render_turntable; use 768 if OOM.
TURNTABLE_RENDER_RESOLUTION = 1024
...
def initialize_mvdiffusion_pipeline(mvdiffusion_checkpoint_path: str, device: torch.device):
    ...
    try:
        ...
        pass  # xformers is CUDA-only; MPS uses native attention
...
def process_single_image(
    ...
    eta: float = 1.0,
    opacity_thres: float = 0.04,
    scaling_thres: float = 0.1,
    floater_thres: float = 0.6,
) -> None:
    ...
    result.gaussians[0].apply_all_filters(
        opacity_thres=opacity_thres,
        scaling_thres=scaling_thres,
        floater_thres=floater_thres,
    ...
    vis_image = render_turntable(
        result.gaussians[0],
        rendering_resolution=TURNTABLE_RENDER_RESOLUTION,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=True,
        help="MTCNN face crop before MV diffusion (default: on). --no-auto-crop if the face is already tightly framed.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scheduler eta for multi-view diffusion (try 0.85–1.0; lower can be slightly cleaner)",
    )
    parser.add_argument(
        "--opacity_thres",
        type=float,
        default=0.04,
        help="Gaussian prune: minimum opacity (default 0.04)",
    )
    parser.add_argument(
        "--scaling_thres",
        type=float,
        default=0.1,
        help="Gaussian prune: maximum world scale (default 0.1)",
    )
    parser.add_argument(
        "--floater_thres",
        type=float,
        default=0.6,
        help="Gaussian prune: floater cull aggressiveness (default 0.6; try 0.5–0.55 for fewer floaters)",
    )
    ...
    main(
        ...
        eta=args.eta,
        opacity_thres=args.opacity_thres,
        scaling_thres=args.scaling_thres,
        floater_thres=args.floater_thres,
    )
